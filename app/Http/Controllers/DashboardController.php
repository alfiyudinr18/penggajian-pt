...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($user->hasRole('admin')) {
            ...
            $draftCount = Penggajian::where('status', 'draft')->count();
            $finalCount = Penggajian::where('status', 'final')->count();

            $data['total_penggajian'] = $totalPenggajian;
            $data['penggajian_draft'] = $draftCount;
            $data['penggajian_final'] = $finalCount;

            // 4. Total Kasbon Aktif
